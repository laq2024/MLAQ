...
SANDWICH_ACTION_SPACE = """[Action Options]
...
"""
    ...
    def get_agent_prompt(self, state_desp) -> str:
        ...
        for line in state_desp_lines:
            for food in self.food_items:
                if food + ': ' in line:
                    line = line.replace("on right side", "on Chad's side")
                    line = line.replace("on left side", "on Dave's side")
        ...
        agent_prompt = f"""You are a multi-agent planner to make a [{self.recipe_name}].
...
Each robot can only interact with items on their own side of the table. Chad for right side, Dave for left side. 
Only one robot can PUT at a time, so two robots must coordinate to stack the food items in the correct order.

[Planning Instruction]
You should think step-by-step to make sure the action is correct. You must follow the following steps: [Thinking Process], [Forbidden Actions], [Conclusion]
[1. Thinking Process]
"Since the next food item should be PUT on the sandwich is ..., and ... The [Reachable Items] are ..., so ..."
[2. Forbidden Actions]
"Check if the chosen action is in the forbidden actions list. If it is, then the action is invalid."
[3. Conclusion]
"Based on the above analysis, the final plan is: EXECUTE\nNAME Chad ACTION ... and NAME Dave ACTION ..."

At the current round:
{food_states}
{agent_states_dict['Chad']}
{agent_states_dict['Dave']}
The table state is:
{table_states}
...
Think step-by-step about the task. Carefully check and correct them if they made a mistake. 
Improve your plans if given feedbacks.
Respond very concisely but informatively. Select from [Action Options], output the final plan with 'EXECUTE\nNAME Chad ACTION ...' and 'NAME Dave ACTION ...'.
"""
        return agent_prompt
