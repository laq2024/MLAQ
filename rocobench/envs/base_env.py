...
class MujocoSimEnv:
    ...
    def __init__(
        ...
        self.name = None
        self.case_number = 10
        self.target_state_desp = None
        self.optimal_steps = 10
    ...
    def reset(self, case_id=0, keyframe_id=0, home_pos=None, reload=False, filepath=None):
        if reload:
            if filepath is None:
                filepath = self.xml_file_path
            assert os.path.exists(
                filepath), f"While attempting to reload from xml file, filepath {filepath} does not exist"
            self.physics = dm_mujoco.Physics.from_xml_path(filepath)
        ...
        if self.randomize_init:
            self.sample_initial_scene(case_id=case_id)
    ...
    def sample_initial_scene(self, case_id=0):
        # task specific!
        return
    ...
    def get_state_prompt(self, obs: EnvState):
        """ Describes the state space for the task """
        raise NotImplementedError

    def get_task_prompt(self):
        """ Describes the task for the agent at the current time step """
        raise NotImplementedError
    ...
    def get_task_feedback(self, obs, llm_plan, pose_dict) -> Tuple[str, dict]:
    ...
    def get_state_space_prompt(self):
        """ Describes the state space for the task """
        raise NotImplementedError
    ...
    def get_target_prompt(self, obs: EnvState) -> str:
        """ Describes the target object for the task """
        raise NotImplementedError
    ...
    def get_examples(self):
        """ Returns a list of example dialogues for the task """
        raise NotImplementedError

    def get_initial_information(self):
        """ Returns initial information for the task """
        raise NotImplementedError
