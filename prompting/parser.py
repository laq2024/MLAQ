...
class LLMResponseParser:
    ...
    def parse_open_action(
        ...
        if "right" in open_target:
            inhand = (
            "cabinet_rightdoor", "right_door_handle", "rightdoorhinge")  # allow collision with both door and handle
        else:
            inhand = (
            "cabinet_leftdoor", "left_door_handle", "leftdoorhinge")  # allow collision with both door and handle
    ...
    def parse_pick_and_place(
        ...
        if place_target_pose is None:
            parsed = f"Target cannot be parsed, bad format."
            return False, parsed, []
        ...
        place_plan = dict(
            ...
            inhand=None,
            # NOTE: tmp issue here, cannot set inhand to (obj_name, obj_site, joint_name) when planning ahead
            action_strs=action_desp,
            return_home=True
        )
...
class TextResponseParser:
    ...
    def parse(self, state: str, action: str):
        return True, '', action
