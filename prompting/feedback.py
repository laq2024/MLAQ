...
class FeedbackManager:
    ...
    def task_feedback(self, obs, llm_plan: LLMPathPlan) -> Tuple[str, dict]:
        task_feedback, error_info = self.env.get_task_feedback(
            obs,
            llm_plan, 
    ...
    def give_feedback(self, obs, llm_plan: LLMPathPlan) -> Tuple[bool, str, dict]:
        """
        ...
        task_feedback, error_info = self.task_feedback(obs, llm_plan)
        plan_passed = True 
        ...
        # breakpoint()
        return plan_passed, feedback, error_info
...
class TextFeedbackManager:
    ...
    def task_feedback(self, obs, llm_plan) -> Tuple[str, dict]:
        task_feedback, error_info = self.env.get_task_feedback(llm_plan)
        return task_feedback, error_info

    def give_feedback(self, obs, llm_plan: str) -> Tuple[bool, str, dict]:
        """
        Given a parsed LLM plan, run task validations and provide feedback if needed
        """
        feedback = f"[Environment Feedback]:\n- Previous Plan:\n{llm_plan}\n"
        task_feedback, error_info = self.task_feedback(obs, llm_plan)
        if error_info['dynamics_error'] or error_info['task_error']:
            plan_passed = False
            feedback += f"Task Constraints:\n failed, {task_feedback}\n"
        else:
            plan_passed = True
