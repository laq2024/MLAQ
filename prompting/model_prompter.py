...
"""
        ...
        system_prompt = f"""You are a centralized planner. You have received a [State] from the external environment. Now, you need to output an action to reach the target [State] from current [State]. Follow the [Detailed Instruction for Policy] strictly.
...
{next_state}"""
    ...
    def get_forbidden_action_prompt(self, forbidden_actions: list):
        ...
        if len(forbidden_actions) == 0:
            ...
            prompt += "I will provide some forbidden joint actions, and the robots cannot choose the forbidden and mistake actions. Note that what is forbidden are the joint actions of the multi-agent system, rather than the single agent actions that appear in these joint actions."
...
{parsed_str} Re-format to strictly follow [Action Output Instruction]!"""
    ...
    def get_llm_action(self, state_desp: str = None, feedbacks: list = [], forbidden_actions: list = [], timestep=None,
        ...
        action = self.env.process_action(response)
